...
— Matrix-free + joblib-parallel version — + LP-BLISS shift + Frobenius-norm truncation
...
Pipeline:
    ...
    4. For k = 1..NumberOfUnitaries:
           ...
           - Optional ≤ n-body truncation (H_trunc_nbody).
           - LP-BLISS shift again:
                 H̄_k → H_BLISS,k = bliss_shift(H_trunc_nbody, N_e).
           - Frobenius-norm no-growth truncation (Pauli or Majorana basis):
                 H_BLISS,k → H_{k+1} (Frobenius-truncated).
           - Save H_{k+1} and continue.
...
Features:
  ...
  • Frobenius-norm truncation:
      - "pauli"    : exact ||ΔH||_F via Pauli-string basis (JW).
      - "majorana" : exact ||ΔH||_F via Majorana-string basis.
      - Used after every BLISS step; the truncated H is passed to the next unitary step.
...
# Transform / truncation behavior (n-body rank truncation)
...
# Frobenius truncation (no-growth) controls (Pauli/Majorana basis)
# FROB_TRUNCATION_MODE:
#   "none"     → skip Frobenius truncation
#   "pauli"    → no-growth truncation using Pauli-string basis (JW)
#   "majorana" → no-growth truncation using Majorana-string basis
FROB_TRUNCATION_MODE    = "majorana"  # "none", "pauli", or "majorana"
FROB_ENERGY_BUDGET_mH   = 1         # ε in mHartree: ||ΔH||_F ≤ ε
FROB_KEEP_CONSTANT      = True        # never drop identity term
FROB_ORDER_BY           = "abscoeff"  # currently only |coeff| ordering is used
...
# ────────── Frobenius-norm truncation (no-growth, Pauli/Majorana) ──────────
#
# We drop fermionic terms in ascending |coeff| order.
# Maintain ΔH (the DROPPED part) in an orthonormal operator basis:
#   • Pauli strings on qubits (JW), or
#   • Majorana strings on fermions.
# Because the basis is HS-orthonormal, ||ΔH||_F^2 = Σ |coeff_basis|^2.
# For each candidate term, expand it in the chosen basis, compute change
# in Σ |coeff|^2, and accept the drop only if we stay within ε^2.
# ---------------------------------------------------------------------------

@dataclass
class TermRecord:
    term: Tuple[Tuple[int, int], ...]        # FermionOperator key
    coeff: complex
    ops: List[Tuple[int, str]]              # (mode, '+' or '-')
    is_const: bool


def build_term_records(fop: FermionOperator) -> List[TermRecord]:
    records: List[TermRecord] = []
    for term, c in fop.terms.items():
        if c == 0:
            continue
        if len(term) == 0:
            records.append(
                TermRecord(term=term, coeff=complex(c), ops=[], is_const=True)
            )
        else:
            ...
            for mode, action in term:
                if action == 1:
                    ...
                    raise ValueError(f"Unknown FermionOperator action {action}")
            records.append(
                TermRecord(term=term, coeff=complex(c), ops=ops, is_const=False)
            )
    return records

# ---------- Majorana expansion ----------

def _multiply_majorana_string(
    indices: Tuple[int, ...],
    m: int,
) -> Tuple[Tuple[int, ...], int]:
    """
    Multiply canonical Majorana string γ_{indices} on the right by γ_m.

    indices: sorted tuple of Majorana indices (0..2*M-1).
    Returns (new_indices, sign), where sign ∈ {+1,-1}.
    """
    L = list(indices)
    n = len(L)
...
def expand_record_to_majorana(
    M: int,
    rec: TermRecord,
) -> Dict[Tuple[int, ...], complex]:
    """
    Expand a single fermionic term into Majorana strings.

    Conventions:
        γ_{2j}   = c_j + c_j^\dagger
        γ_{2j+1} = -i(c_j - c_j^\dagger)

    Inverse:
        c_j         = (γ_{2j} + i γ_{2j+1}) / 2
        c_j^\dagger = (γ_{2j} - i γ_{2j+1}) / 2

    Returns dict: majorana_index_tuple -> coefficient.
    """
    strings: Dict[Tuple[int, ...], complex] = {(): complex(rec.coeff)}

    for mode, op in rec.ops:
        new_strings: Dict[Tuple[int, ...], complex] = {}
        if op == "-":  # annihilation c_j
            combos = [(0.5 + 0j, 2 * mode), (0.5j, 2 * mode + 1)]
        elif op == "+":  # creation c_j^\dagger
            combos = [(0.5 + 0j, 2 * mode), (-0.5j, 2 * mode + 1)]
        else:
            raise ValueError(f"Invalid fermionic op {op}")

        for idxs, c in strings.items():
            for local_coeff, maj_idx in combos:
                new_idx, sign = _multiply_majorana_string(idxs, maj_idx)
                new_coeff = c * local_coeff * sign
                new_strings[new_idx] = new_strings.get(new_idx, 0.0) + new_coeff

        strings = {k: v for k, v in new_strings.items() if abs(v) > 1e-12}

    return strings

# ---------- Pauli (Jordan–Wigner) expansion ----------
...
def _multiply_paulis(p1: str, p2: str) -> Tuple[str, complex]:
    """Multiply single-site Pauli operators."""
    try:
        return _PAULI_MULT[(p1, p2)]
    except KeyError:
        raise ValueError(f"Invalid Pauli product {p1}{p2}")

def _multiply_pauli_strings(
    ps1: Tuple[complex, Dict[int, str]],
    ps2: Tuple[complex, Dict[int, str]],
) -> Tuple[complex, Dict[int, str]]:
    """
    Multiply two Pauli strings.

    Representation:
        (coeff, {qubit_idx: 'X'/'Y'/'Z'})
    Missing indices imply 'I'.
    """
    coeff1, ops1 = ps1
    coeff2, ops2 = ps2
    coeff = coeff1 * coeff2
    ...
    for q, p2 in ops2.items():
        p1 = ops.get(q, "I")
        if p1 == "I":
            ops[q] = p2
        else:
            p, phase = _multiply_paulis(p1, p2)
            coeff *= phase
            if p == "I":
                ops.pop(q, None)
            else:
...
def _map_ladder_to_pauli(j: int, op: str) -> List[Tuple[complex, Dict[int, str]]]:
    """
    Jordan–Wigner map for a single ladder operator on mode j:

        c_j         = (prod_{k<j} Z_k) * (X_j + i Y_j) / 2
        c_j^\dagger = (prod_{k<j} Z_k) * (X_j - i Y_j) / 2
    """
    terms: List[Tuple[complex, Dict[int, str]]] = [(1.0 + 0j, {})]

    # Z string on modes < j
    for k in range(j):
        z_term = (1.0 + 0j, {k: "Z"})
        new_terms: List[Tuple[complex, Dict[int, str]]] = []
        for t in terms:
            new_terms.append(_multiply_pauli_strings(t, z_term))
        terms = new_terms

    if op == "-":  # annihilation
        locals_ = [(0.5 + 0j, {j: "X"}), (0.5j, {j: "Y"})]
    elif op == "+":  # creation
        locals_ = [(0.5 + 0j, {j: "X"}), (-0.5j, {j: "Y"})]
    else:
        raise ValueError(f"Invalid fermionic op {op}")
    ...
    for t in terms:
        for l in locals_:
            new_terms.append(_multiply_pauli_strings(t, l))

    accum: Dict[Tuple[Tuple[int, str], ...], complex] = defaultdict(complex)
    for coeff, ops in new_terms:
        key = tuple(sorted(ops.items()))
        accum[key] += coeff

    return [(c, dict(k)) for k, c in accum.items() if abs(c) > 1e-12]

def expand_record_to_pauli(
    rec: TermRecord,
) -> Dict[Tuple[Tuple[int, str], ...], complex]:
    """
    Expand a single fermionic term into Pauli strings via Jordan–Wigner.

    Returns dict: pauli_key -> coefficient,
    where pauli_key is tuple(sorted({qubit: 'X'/'Y'/'Z'}.items())).
    """
    terms: List[Tuple[complex, Dict[int, str]]] = [(rec.coeff + 0j, {})]

    for mode, op in rec.ops:
        local_terms = _map_ladder_to_pauli(mode, op)
        new_terms: List[Tuple[complex, Dict[int, str]]] = []
        for t in terms:
            for lt in local_terms:
                new_terms.append(_multiply_pauli_strings(t, lt))
        ...
        # FIX: iterate as (key, coeff), not (coeff, ops)
        terms = [
            (coeff, dict(key))
    ...
    return {tuple(sorted(ops.items())): coeff for coeff, ops in terms}

def frobenius_truncate(
    fop: FermionOperator,
    mode: str,
    eps_H: float,
    keep_constant: bool = True,
    order_by: str = "abscoeff",
    M: Optional[int] = None,
):
    """
    No-growth Frobenius truncation in either Pauli or Majorana basis.
    ...
    mode : str
        'pauli' or 'majorana'.
    eps_H : float
        Target normalized Frobenius bound ||ΔH||_F ≤ eps_H (in Hartree).
    keep_constant : bool
        Whether to forbid dropping the identity term.
    order_by : str
        Ignored for now; we always sort by |coeff|.
    M : int, optional
        Number of spin-orbitals (qubits); if None, inferred from fop.

    Returns
    -------
    kept_op : FermionOperator
        Truncated Hamiltonian (normal-ordered).
    info : dict
        Summary: n_total, n_kept, n_dropped, delta_fro^2, delta_fro, mode.
    """
    mode = (mode or "none").lower()
    if mode not in ("pauli", "majorana"):
        raise ValueError("frobenius_truncate: mode must be 'pauli' or 'majorana'.")

    if M is None:
        M = count_qubits(fop)

    records = build_term_records(fop)
    n_total = len(records)
    eps2 = eps_H * eps_H

    if order_by != "abscoeff":
        print(f"[FROB] warning: order_by='{order_by}' ignored; using 'abscoeff'.")

    records_sorted = sorted(records, key=lambda r: (abs(r.coeff), len(r.ops)))

    if mode == "pauli":
        basis_coeffs: Dict[Tuple[Tuple[int, str], ...], complex] = {}
    else:
        basis_coeffs: Dict[Tuple[int, ...], complex] = {}

    frob2_err = 0.0
    dropped_terms = set()

    for rec in records_sorted:
        if keep_constant and rec.is_const:
            continue

        if mode == "pauli":
            exp = expand_record_to_pauli(rec)
        else:
            exp = expand_record_to_majorana(M, rec)

        inc = 0.0
        old_vals: Dict[object, complex] = {}
        for key, delta in exp.items():
            old = basis_coeffs.get(key, 0.0)
            old_vals[key] = old
            new = old + delta
            inc += (abs(new) ** 2 - abs(old) ** 2)

        new_frob2 = frob2_err + inc

        if new_frob2 <= eps2 + 1e-16:
            for key, delta in exp.items():
                basis_coeffs[key] = old_vals[key] + delta
            frob2_err = new_frob2
            dropped_terms.add(rec.term)

    kept = FermionOperator()
    for rec in records:
        if rec.term in dropped_terms:
            continue
        kept += FermionOperator(rec.term, rec.coeff)

    n_kept = sum(1 for _, c in kept.terms.items() if c != 0)
    delta_fro = math.sqrt(max(frob2_err, 0.0))
    ...
    info = dict(
        ...
        delta_fro2=frob2_err,
        delta_fro=delta_fro,
    )
    return normal_ordered(kept), info
...
def make_screening_support_small_tail(H: FermionOperator, tail_l1: float):
    """
    Build the 'screening' support of H via smallest-|coeff| tail.
    """
    if not H.terms:
        return set(), 0.0, 0
    if tail_l1 <= 0.0:
...
def solve_lp_l1_sparse(
    ...
    """
    Solve min_θ ||Hr - Dr θ||₁ + ||Hi - Di θ||₁ with "no new terms" constraints.
    """
    n, P = Dr.shape
    ...
    if new_term_indices.size > 0:
        ...
        zeros_res = sp.csc_matrix((Aeq_extra.shape[0], 4 * n))
        A_eq = sp.vstack(
...
if __name__ == "__main__":
    ...
    if TRUNCATION_N_BODY is None:
        print("Rank truncation: NONE (full Ḣ will be reused before BLISS)\n")
    else:
        mode = "number-conserving" if TRUNCATION_NUMBER_CONSERVING else "non-conserving bound"
        print(f"Rank truncation: ≤ {TRUNCATION_N_BODY}-body ({mode})\n")

    frob_mode_display = (FROB_TRUNCATION_MODE or "none").lower()
    if frob_mode_display in ("pauli", "majorana"):
        print(
            f"Frobenius truncation after each BLISS: "
            f"mode={frob_mode_display} (basis), "
            f"ε={FROB_ENERGY_BUDGET_mH} mH, "
            f"keep_constant={FROB_KEEP_CONSTANT}\n"
        )
    else:
        print("Frobenius truncation: DISABLED\n")

    GLOB["ket_sparse"] = ket_HF
    GLOB["bit_idx"]    = bit_idx
    ...
    for k in range(1, NumberOfUnitaries + 1):
        ...
        print(f"Ḣ term count (pre-rank-truncate): {len(H_bar.terms)}")
        E_bar = expect_on_HF(H_bar)
        print(f"⟨Φ_HF|Ḣ|Φ_HF⟩ (pre-rank-truncate): {E_bar:.12f} Ha")

        if TRUNCATION_N_BODY is None:
            H_trunc = normal_ordered(H_bar)
            E_trunc = E_bar
            print("No rank truncation selected → using full Ḣ as input to BLISS / Frobenius.")
        else:
        ...
        frob_mode = (FROB_TRUNCATION_MODE or "none").lower()
        if frob_mode in ("pauli", "majorana"):
            print("\n[FROB] Running Frobenius-norm no-growth truncation on H_BLISS …")
            H_frob, frob_info = frobenius_truncate(
                H_bliss,
                mode=frob_mode,
                eps_H=FROB_ENERGY_BUDGET_mH * 1e-3,
                keep_constant=FROB_KEEP_CONSTANT,
                order_by=FROB_ORDER_BY,
                M=n_qb,
            )
            E_frob = expect_on_HF(H_frob)
            print(f"[FROB] mode={frob_info['mode']}  |  ε={FROB_ENERGY_BUDGET_mH} mH")
            print(f"[FROB] dropped terms: {frob_info['n_dropped']}")
            dH = frob_info["delta_fro"]
            print(f"[FROB] ||ΔH||_F (basis) = {dH:.6e} Ha  (~ {1000*dH:.6e} mH)")
            print(f"[FROB] terms after truncation : {frob_info['n_kept']}")
            print(f"[FROB] ⟨Φ_HF|H_FROB|Φ_HF⟩  : {E_frob:.12f} Ha")

            H_next = normal_ordered(H_frob)
        else:
            print("\n[FROB] Frobenius truncation disabled → using H_BLISS directly.")
        ...
        if SAVE_OUT:
            ...
            print(f"✓ Saved transformed+BLISS+FROB Hamiltonian → {out_path}")

        H = H_next

    print("\nAll requested unitaries processed (initial BLISS + [rank truncation] + BLISS + Frobenius truncation per step).")
